...
/*****************************************************************************
 * Function: main
 * Description: Program used to draw a graphiz representation of a Huffman
 *             tree.
 * Inputs: Reads from file 'tree.txt'
 * Outputs: None 
 * Time Complexity: O(?)
 * Space Complexity: O(?)
 * Usage: ./drawTree | dot -Tpng -o tree.png
 *****************************************************************************/
int main(int argc, char** argv) {
 
  FILE* tree = fopen("tree.txt", "rb");
  if (tree == NULL) {
    printf("Error: could not find a tree.txt file\n");
    return 1;
  }

  HuffmanTreeNode* root = readTree(tree);
  fclose(tree);

  printTreeGraphiz(root);
  freeTree(root);
